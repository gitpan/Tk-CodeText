...
sub rulesEdit {
	my $cw = shift;
#	print "rulesEdit is called\n";
	my $rl = $cw->cget('-rules');
	if (not defined $rl) {
		$rl = $cw->highlightPlug->rules;
	}
	if (defined($rl)) {
		my @rules = @$rl;
		...
		my $lf = $db->LabFrame(
			-label => 'Tags',
			-labelside => 'acrosstop',
		)->pack(-side => 'left', -fill => 'y', -padx => 2, -pady => 2);
		my $l = $lf->Scrolled('Listbox',
			-height => 4,
			-scrollbars => 'osoe',
			-exportselection => 0,
		)->pack(-fill => 'y', -expand => 1, -padx => 2, -pady => 2);
		foreach my $tn (@rules) {
			$l->insert('end', $tn->[0])
		}
		my $f = $db->Frame(
		...
		my @lop = (-labelPack => [-side => 'left', -anchor => 'w']);
		my @lpk = (-padx => 2, -pady => 2, -anchor => 'ne');
		my $fg = ''; my $bg = ''; my $fm = ''; my $sz = ''; my $wt = ''; my $st;
		$cf->LabEntry(@lop, -label => 'Foreground', -textvariable => \$fg,)->pack(@lpk);
		$cf->LabEntry(@lop, -label => 'Background', -textvariable => \$bg,)->pack(@lpk);
		my $ff = $f->LabFrame(
			-label => 'Font',
			-labelside => 'acrosstop',
		)->pack(-padx => 2, -pady => 2, -fill => 'both');
		$ff->LabEntry(@lop, -label => 'Family', -textvariable => \$fm,)->pack(@lpk);
		$ff->LabEntry(@lop, -label => 'Size', -textvariable => \$sz,)->pack(@lpk);
		$ff->LabEntry(@lop, -label => 'Weight', -textvariable => \$wt,)->pack(@lpk);
		$ff->LabEntry(@lop, -label => 'Slant', -textvariable => \$st,)->pack(@lpk);
		my $current;
		$updatesub = sub {
			my $s = $l->curselection;
			if (defined($s)) {
				if (defined($current)) {
					my @nop = ($rules[$current]->[0]);
					my @fop = ();
					if ($fg) { push(@nop, -foreground => $fg) };
					if ($bg) { push(@nop, -background => $bg) };
					if ($fm) { push(@fop, -family => $fm) };
					if ($sz) { push(@fop, -size => $sz) };
					if ($wt) { push(@fop, -weight => $wt) };
					if ($wt) { push(@fop, -slant => $st) };
					if (@fop) {
						push(@nop, -font => [@fop]);
					}
					$rules[$current] = [@nop];
#					use Data::Dumper; print Dumper(@rules);
				...
				print "doing $s\n";
				$current = $s;
				my $c = $rules[$current];	my @op = @$c; shift @op;
				$bg = ''; $fg = ''; $fm = ''; $sz = ''; $wt = '';
				while (@op) {
					my $option = shift @op;
					my $value = shift @op;
					if ($option eq '-background') {
						$bg = $value
					} elsif ($option eq '-foreground') {
						$fg = $value
					} elsif ($option eq '-font') {
						my @fop = @$value;
						while (@fop) {
							my $foption = shift @fop;
							my $fvalue = shift @fop;
							if ($foption eq '-family') {
								$fm = $fvalue
							} elsif ($foption eq '-size') {
								$sz = $fvalue
							} elsif ($foption eq '-weight') {
								$wt = $fvalue;
							};
						};
					};
